...
class RunnerPage
{
	...
	function assignBodyEnd($params = "")
	{
		...
		echo "<script language=\"JavaScript\" src=\"".GetRootPathForResources("include/runnerJS/RunnerAll.js?40776")."\"></script>\r\n";
	...
	function buildPaginationControl( $currentPage, $maxPages, $limit ) {
		...
		if($counterstart != 1)
		{
			$pagination.= $this->getPaginationLink(1,"أول");
			$pagination.= $this->getPaginationLink($counterstart - 1,"سابق");
		}
		...
		if( $counterend != $maxPages ) {
			$pagination.= $this->getPaginationLink($counterend + 1,"تال");
			$pagination.= $this->getPaginationLink($maxPages,"أخير");
		}
	...
	public function assignSimpleSearch() {
		...
		$this->xt->assign('searchbutton_attrs', 'id="searchButtTop'.$this->id.'" title="'."بحث".'"');
	 ...
	 */
	 protected function assignSearchControl() {
		...
		$searchforAttrs.= ' placeholder="'."بحث".'"';
...
//		$settings["requestDelay"] = $option["forceExpire"] && $option["lifeTime"] < 2 ? 30 : 60;
	...
	*/
	...
	protected function getFieldFilterStateEntry( $fieldName, $value, $selectedValuesSet ) {
		...
		$displayValue = $displayValue !== "" ? $displayValue : "(" . "تفريغ" . ")";
