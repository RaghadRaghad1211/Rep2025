...
{	
	...
	function  getDelButtonHtml($fName, $recId)
	{
		...
		$html = '<a id = "'.$this->getDelButtonId($fName, $recId).'" ctrlId="'.$recId.'" fName="'.GoodFieldName($fName)
			.'" class="searchPanelButton" href="#" title="'."حذف الرقابة".'">' . $text . '</a>';
		return $html;
	}
	...
	public static function getSimpleSearchTypeCombo( $selOpt, $not, $id ) {
		$options = "<option value=\"Contains\" ".( $selOpt == "Contains" && !$not ? "selected" : "" ).">"."يحتوي"."</option>";
		$options.= "<option value=\"Equals\" ".( $selOpt == "Equals" && !$not ? "selected" : "" ).">"."يساوي"."</option>";
		$options.= "<option value=\"Starts with\" ".( $selOpt == "Starts with" && !$not ? "selected" : "" ).">"."يبدأ بـ ..."."</option>";
		$options.= "<option value=\"More than\" ".( $selOpt == "More than" && !$not ? "selected" : "" ).">"."أكثر من ..."."</option>";
		$options.= "<option value=\"Less than\" ".( $selOpt == "Less than" && !$not ? "selected" : "" ).">"."أقل من ..."."</option>";
		$options.= "<option value=\"Empty\" ".( $selOpt == "Empty" && !$not ? "selected" : "" ).">"."تفريغ"."</option>";
	...
	public static function simpleSearchFieldCombo( $fNamesArr, $googleLikeFields, $selOpt, $tName, $id ) {
		$options = "";
		if( $googleLikeFields )
			$options = '<option value="" >'."أي حقل".'</option>';
