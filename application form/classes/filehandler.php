...
class RunnerFileHandler {
	...
	protected function validateFile( $filename, $fileSize, &$errorMessage ) {
        if ( !$filename ) {
            $errorMessage = "لم يتم توفير اسم الملف";
            return false;
        }
		...
		if( $acceptedTypes ) {
			...
			if( array_search( $ext, $acceptedTypes ) === false && array_search( ".".$ext, $acceptedTypes ) === false ) {
				$errorMessage = "نوع الملف غير مقبول";
				return false;
			}
		}
		...
		if ( $maxFileSize && $fileSize > $maxFileSize * 1024 ) {
            $errorMessage = mysprintf("حجم الملف يتجاوز حد كيلوبايت٪ S", array( $maxFileSize ) );
            return false;
        }
		...
		if ( $maxTotalFileSize ) {
			if( $this->getUploadFilesSize() + $fileSize > $maxTotalFileSize * 1024 ) {
				$errorMessage = mysprintf( "إجمالي الملفات حجم يتجاوز حد كيلوبايت٪ S", array( $maxTotalFileSize ) );
				return false;
			}
		}
		...
		if ( $maxNumberOfFiles && $this->getUploadFilesCount() >= $maxNumberOfFiles ) {
			$errorMessage = mysprintf( "يمكنك تحميل أي ملفات أكثر من٪ S", array( $maxNumberOfFiles ) );
			return false;
		}
        return true;
	}
